...
import type { MakerWorldModel } from "@/lib/types";
...
export function CalculatorApp({
  ...
  const costs: CostBreakdown | null = useMemo(() => {
    ...
    return calculateCosts(weightGrams, printTimeSeconds, pricing);
  }, [weightGrams, printTimeSeconds, pricing]);

  const qty = Math.max(1, Math.floor(quantity) || 1);

  const showBulkPrice =
    costs?.bulkEligible && costs.bulkPrice !== null && qty >= pricing.bulkQuantity;

  const tier = costs ? activePriceTier(costs, qty, pricing.bulkQuantity) : "retail";

  const orderTotal = costs
    ? unitPriceForQuantity(costs, qty, pricing.bulkQuantity) * qty
    : 0;
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8 md:py-12">
      ...
      {model && costs ? (
        <div className="space-y-5">
          {isLongPrint(costs.printTimeHours, longPrintHours) ? (
            <div className="warning-banner rise-in" role="alert">
              <span aria-hidden>⚠</span>
              <p>
                <strong>Impresión larga:</strong> esta pieza tarda más de {longPrintHours}{" "}
          ...
          <section className="panel rise-in space-y-4 p-5 md:p-6">
            ...
            <label className="block space-y-1.5">
              ...
              <span className="block text-xs text-muted">
                Con {pricing.bulkQuantity}+ unidades de productos baratos se activa el precio por
                volumen
              </span>
              <input
                type="number"
          ...
          >
            ...
            {showBulkPrice ? (
              <PriceCard
                ...
                subtitle={`${pricing.bulkQuantity}+ unidades`}
          ...
          <section className="panel rise-in p-5 text-center md:p-6">
            ...
            {isLongPrint(costs.printTimeHours * qty, longPrintHours) ? (
